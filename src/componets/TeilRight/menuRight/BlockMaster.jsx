...
function ShowColumns({ block, columnsOfBlock, tooRead, index }) {
  const { arrayOfBlocks } = useContext(MyContext);
        ...
        columnsOfBlock !== undefined
          ...
          columnsOfBlock.map((col, index) => {
            if (!tooRead) {
              return (
                <>
                  <div key={index} ref={referenceLine} className="row p-0 my-1 graycolor100 shadow-sm">
                    <ColAddLine
                      block={block}
                      setArrayColumnsLocal2={setArrayOfColumnsLocal}
                      col={col}
...
// function ButtonIconBlock({ styleCircleCSS, iconComponent, block }) {
